...
def calculate_canvas(n: int):
    ...
    size     = max(20, math.floor(200 * ratio))
...
async def place_pixel(data: PlacePixelRequest):
    ...
    if data.room_id:
        # Room mode — get room canvas size first
        room = supabase.table('rooms').select('cooldown_seconds, canvas_width, canvas_height').eq('id', data.room_id).execute()
        if room.data:
            cooldown_s = room.data[0]['cooldown_seconds']
            canvas_w   = room.data[0]['canvas_width']
            canvas_h   = room.data[0]['canvas_height']
        # Now validate coordinates against actual room canvas size
        ...
        # Global mode — validate bounds against global canvas size
        if data.x < 0 or data.x >= canvas_w or data.y < 0 or data.y >= canvas_h:
            raise HTTPException(status_code=400, detail=f'Coordinates out of bounds ({canvas_w}×{canvas_h})')
