...
async def place_pixel(data: PlacePixelRequest):
    ...
    if user['last_pixel_at']:
        last    = datetime.fromisoformat(user['last_pixel_at'].replace('Z', '+00:00').replace('+00:00+00:00', '+00:00'))
    ...
    # Update cooldown timestamp and pixel count together
    supabase.table('participants') \
        .update({ 'last_pixel_at': now.isoformat() }) \
        .eq('fest_code', data.fest_code) \
        .execute()

    # Increment pixel count safely
    try:
